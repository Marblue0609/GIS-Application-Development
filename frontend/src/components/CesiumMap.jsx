...
// 紫金港周边初始相机范围：[西, 南, 东, 北]
const initialView = Cesium.Rectangle.fromDegrees(
  120.055,
  30.285,
  120.105,
  30.326,
...
const almostSamePoint = (a, b, tolerance = 0.00018) => {
  ...
  if (!Number.isFinite(pointA.lng) || !Number.isFinite(pointA.lat))
    return false;
  if (!Number.isFinite(pointB.lng) || !Number.isFinite(pointB.lat))
    return false;
  return (
    Math.abs(pointA.lng - pointB.lng) < tolerance &&
...
const withFocusedItem = (items, focusedItem) => {
  ...
  return [
    ...items.filter((item) => !sameMapItem(item, focusedItem)),
    focusedItem,
  ];
};

const withPinnedItems = (items, pinnedItems) =>
  pinnedItems.reduce(
    (currentItems, item) => withFocusedItem(currentItems, item),
    items,
  );

const uniqueMapItems = (items) =>
...
const findMatchingMapItem = (items, target) =>
  items.find((item) => sameMapItem(item, target)) ?? target;

const isInMapItems = (items, target) =>
  items.some((item) => sameMapItem(item, target));
...
const boostColor = (color, amount = 0.14) => {
  ...
  const channel = (value) =>
    Math.min(1, Math.max(0, avg + (value - avg) * (1 + amount)));
...
const createPinIcon = (item, color, selected = false) => {
  ...
  const glyph =
    layerType === "restaurant"
      ? restaurantGlyph(category)
      : supportGlyph(layerType);
...
const displayConfig = (viewMetrics, layerType) => {
  ...
  return (
    (presets[layerType] ?? presets.restaurant).find(
      (preset) => effectiveSpan > preset.span,
    ) ?? {
      cellSize: 0,
      maxCount: Number.POSITIVE_INFINITY,
      minPixels: layerType === "landmark" ? 44 : 28,
    }
  );
};
...
const itemPriority = (item, layerType) => {
  if (layerType === "restaurant") {
    return (
      (Number(item.rating) || 0) * 100 + Math.min(Number(item.price) || 0, 120)
    );
  }
...
const decimateByGeoGrid = (items, config, layerType) => {
  ...
  items.forEach((item) => {
    ...
    if (
      !current ||
      itemPriority(item, layerType) > itemPriority(current, layerType)
    ) {
...
const screenPixelFromItem = (viewer, item) => {
  const { lng, lat } = getLngLat(item);
  if (!viewer || !Number.isFinite(lng) || !Number.isFinite(lat)) return null;
...
const screenDistanceBelow = (a, b, maxDistance) => {
  if (!a || !b) return false;
  const dx = a.x - b.x;
  const dy = a.y - b.y;
...
const removeDataSourcesByName = (viewer, name) => {
  viewer.dataSources.getByName(name).forEach((dataSource) => {
...
function CesiumMap({
  ...
  useEffect(() => {
    ...
    removeDataSourcesByName(viewer, "restaurants");
    ...
    const focusedRestaurant =
      selectedMapItem?.layerType === "restaurant"
        ? selectedMapItem
        : restaurants.find((item) => sameId(item.id, focusedRestaurantId));
    visibleRestaurants = uniqueMapItems(
      withFocusedItem(
        withPinnedItems(visibleRestaurants, routeRestaurants),
        focusedRestaurant,
      ),
    );
    if (!activeCategory) {
      visibleRestaurants = avoidScreenOverlap(
        viewer,
        visibleRestaurants,
        config.minPixels,
        "restaurant",
    ...
    visibleRestaurants.forEach((restaurant) => {
      const color = boostColor(
        colorFromCategory(restaurant.categoryGroup ?? restaurant.category),
      );
    ...
    return () => {
      ...
      if (restaurantDataSourceRef.current === dataSource) {
        restaurantDataSourceRef.current = null;
      }
    };
  }, [
    restaurants,
  ...
  useEffect(() => {
    ...
    removeDataSourcesByName(viewer, "support-points");
    ...
    let visibleLandmarks = decimateByGeoGrid(
      landmarks,
      landmarkConfig,
      "landmark",
    );
    let visibleTransportations = decimateByGeoGrid(
      transportations,
      transportationConfig,
      "transportation",
    );
    const focusedLandmark =
      selectedMapItem?.layerType === "landmark"
        ? selectedMapItem
        : landmarks.find((item) => sameId(item.id, focusedRestaurantId));
    const focusedTransportation =
      selectedMapItem?.layerType === "transportation"
        ? selectedMapItem
        : transportations.find((item) => sameId(item.id, focusedRestaurantId));

    visibleLandmarks = uniqueMapItems(
      withFocusedItem(visibleLandmarks, focusedLandmark),
    );
    visibleTransportations = uniqueMapItems(
      withFocusedItem(visibleTransportations, focusedTransportation),
    );

    visibleLandmarks = avoidScreenOverlap(
      viewer,
      visibleLandmarks,
      landmarkConfig.minPixels,
      "landmark",
      focusedLandmark?.id,
    );
    ...
    visibleLandmarks.forEach((item) =>
      createPointEntity(dataSource, item, landmarkColor.withAlpha(1), 27),
    );
    visibleTransportations.forEach((item) =>
      createPointEntity(dataSource, item, transportationColor.withAlpha(1), 23),
    );
    ...
    return () => {
      ...
      if (supportDataSourceRef.current === dataSource) {
        supportDataSourceRef.current = null;
      }
    };
  }, [
    landmarks,
  ...
  useEffect(() => {
    ...
    removeDataSourcesByName(viewer, "analysis-overlays");
    ...
    if (analysisArea) {
      overlay.entities.add({
        ...
        ellipse: {
          ...
          outline: true,
          outlineColor:
            Cesium.Color.fromCssColorString("#2f6fb3").withAlpha(0.85),
          height: 0,
        },
      });
    }
      ...
      overlay.entities.add({
        ...
        polyline: {
          positions: Cesium.Cartesian3.fromDegreesArray(
            routePath.flatMap((item) => [item.lng, item.lat]),
          ),
          width: 5,
    ...
    return () => {
      if (!viewer.isDestroyed()) {
        viewer.dataSources.remove(overlay, true);
      }
      if (overlayDataSourceRef.current === overlay) {
  ...
  useEffect(() => {
    ...
    const hoveredPixel = screenPixelFromItem(viewer, hoveredItem);

    allEntities.forEach((entity) => {
      const isSelected = sameMapItem(entity.mapItem, selectedMapItem);
      const isRoutePoint =
        entity.mapItem?.layerType === "restaurant" &&
        isInMapItems(routeWaypoints ?? [], entity.mapItem);
      const isHovered = sameMapItem(entity.mapItem, hoveredItem);
      if (!entity.billboard) return;

      const emphasized = isSelected || isRoutePoint;
      const entityPixel = screenPixelFromItem(viewer, entity.mapItem);
      const hiddenBehindHover = Boolean(
        hoveredPixel &&
        hoveredItem &&
        !isHovered &&
        !emphasized &&
        screenDistanceBelow(entityPixel, hoveredPixel, entity.baseWidth + 18),
      );
      entity.show = !hiddenBehindHover;
      entity.billboard.width = emphasized
        ? entity.baseWidth + 12
        : isHovered
          ? entity.baseWidth + 6
          : entity.baseWidth;
      entity.billboard.height = emphasized
        ? entity.baseHeight + 15
        : isHovered
          ? entity.baseHeight + 8
          : entity.baseHeight;
    ...
    <div className="cesium-map-wrap">
      ...
      {hoveredItem && (
        ...
        >
          ...
          {hoveredItem.layerType === "restaurant" && (
            <span>评分 {hoveredItem.rating || "-"}</span>
          )}
        </div>
      )}
    </div>
